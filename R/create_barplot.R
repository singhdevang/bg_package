  ...
  if (orientation == "vertical") {
    p <- p +
      theme(
        ...
        axis.ticks.y = element_blank()  # Hide Y axis ticks as well
      )
    ...
    p <- p +
      ...
      theme(
        ...
        axis.ticks.x = element_blank()  # Hide X axis ticks as well
      )
    if (is.numeric(data[[x]])) {
      p <- p + scale_x_continuous(expand = expansion(mult = c(0, 0))) # Adjusting expansion here for numerical data
    } else {
      p <- p + scale_x_discrete(expand = expansion(mult = c(0, 0))) # Adjusting expansion here for categorical data
    }
  }
